...
2. Collector polls CloudHub APIs for:
   ...
   - raw log lines for each run (used to infer actual runtime outcome)
...
For log-based status extraction, configure `CLOUDHUB_LOGS_API` (supports placeholders `{org_id}`, `{env_id}`, `{app_name}`, `{run_id}`, `{start_ms}`, `{end_ms}`).
...
When logs are available, the collector sets `job.status` from log evidence (`FAILED`/`SUCCESS`/`RUNNING`) and keeps the original API-reported status under `job.cloudhub_api_status`.
...
- If run-status APIs are delayed/inaccurate, the collector derives the effective status from CloudHub logs and attaches the relevant error line.
